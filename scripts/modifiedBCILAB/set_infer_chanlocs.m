...
%                                2010-11-05
% Annotation by NB: In order to be able to load into our lab-internal channel location files, 
% a few code lines had to been modified within this script
% The respective changes are indicated by the "Mod"-comments.
...
if (~all(isfield(locs,{'X','Y','Z'})) || all(cellfun('isempty',{locs.X}))) && ~isempty(locs)
    ...
    % Mod (Niclas & Martin): go through a variety of caps and retain the best (i.e. largest) match
    % Mod: We have added the Oldenburg equidistant cap layout 
    ...
    caps = {'resources:/Standard-10-5-Cap385.sfp','resources:/sccn_LSIE_cap128.xyz','resources:/sccn_BEM_coregistered_128_v2.xyz','resources:/sccn_BEM_coregistered_256_v1.xyz','resources:/sccn_BEM_coregistered_128_v1.xyz','resources:/59_Niclas_equidistant_selection.xyz','resources:/niclas_mobile24.xyz'};
    % Mod: Here we added the name of our channel layout
    schemes = {'10-20','sccn_128_v3','sccn_128_v2','sccn_256_v1','sccn_128_v1','59_Niclas_equidistant_selection','niclas_mobile24'};
    
    % Mod: This line has been added by one further entry
    %matching = {'nocase','substr','substr','substr'};
    % per cap layout one entry in the matching variable is necessary. 
    matching = {'nocase','substr','substr','substr','substr','substr','substr'};
    ...
    % Mod: In order to varify to the User which channel layout has been
    % finally chosen:
    display(['Detected this channel layout ' caps{c} '. Going to use that'])
    try 
